...
设置页支持“导出配置链接”。教师配置好 Endpoint、API Key、模型、AI 模式和顶部菜单显示配置后，点击“生成链接”，系统会生成一个包含 Base64 编码配置的 URL，并尽量自动复制到剪贴板。
...
- 切换“本地模式”和“大语言模型模式”。
- 隐藏或显示“课堂脉络 / 学生问 AI / AI 问学生 / 备课模式”顶部菜单。
- 导出一个可自动导入的 AI 配置链接。
...
页面中的“备课模式”用于制作和发布新课文模板：

1. 输入备课 Token，点击“校验 Token”确认权限。
2. 在“生成导学案”中粘贴课文原文和简要思路，生成导学案模板；也可以在“粘贴导学案”中直接粘贴已有 Markdown 并解析。
3. 进入“编辑模板”后，可调整标题、学习目标、问题提示、AI 提问、预设问答和导学案 Markdown。
4. 点击“保存并推送草稿分支”会提交到 `PREP_GIT_BRANCH`，默认 `lesson-drafts`，不会触发正式部署。
5. 确认草稿后点击“加载草稿分支”，选择文章并点击“合并到 v2”，系统会推送到 `PREP_GIT_TARGET_BRANCH`。
